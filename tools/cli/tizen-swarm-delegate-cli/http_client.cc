...
// Append newline-separated header string to an slist
curl_slist* AppendHeaders(curl_slist* list,
  ...
  while ((pos = h.find('\n')) != std::string::npos) {
    ...
    if (!line.empty()) list = curl_slist_append(list, line.c_str());
    h = h.substr(pos + 1);
  }
  if (!h.empty()) list = curl_slist_append(list, h.c_str());
  return list;
}
...
HttpClient::Response HttpClient::Get(
  ...
  if (!extra_headers.empty()) {
    slist_guard.list = AppendHeaders(slist_guard.list, extra_headers);
    if (slist_guard.list)
      curl_easy_setopt(curl, CURLOPT_HTTPHEADER, slist_guard.list);
  }
...
HttpClient::Response HttpClient::Post(
  ...
  CurlSlistGuard slist_guard;
  slist_guard.list = curl_slist_append(nullptr, "Content-Type: application/json");
  if (!extra_headers.empty())
    slist_guard.list = AppendHeaders(slist_guard.list, extra_headers);
  curl_easy_setopt(curl, CURLOPT_HTTPHEADER, slist_guard.list);
